...
def generate_speed_shrines_with_offset(ctx):
    ...
    for team, p_base, p_target in shrine_configs:
        ...
        create_height_adapted_road(f"ShrinePath_{team}", mid_point, shrine_pos, 10.0, ctx.get_material("road"), ctx)

def generate_3_health_relics(ctx):
    cfg = ctx.config
    crown_dir_outward = Vector((ctx.layout["Crown"].x, ctx.layout["Crown"].y, 0.0)).normalized()
    crown_relic_pos = ctx.layout["Crown"] + crown_dir_outward * (cfg["capture_platform_radius"] + 8.75)
