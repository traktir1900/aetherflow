...
The half-oval is 15% larger than the current enlarged 28x18 m pocket:
32.2 x 20.7 m.
"""
import math
import bmesh
...
def _smooth_rise(ctx, center, ground_z):
    ...
    for j in range(rings + 1):
        ...
        for i in range(segs):
            ...
            row.append(bm.verts.new((center.x + r * math.cos(a),
                                     center.y + r * math.sin(a), z)))
        rings_v.append(row)
    for j in range(rings):
        for i in range(segs):
            ni = (i + 1) % segs
            bm.faces.new((rings_v[j][i], rings_v[j][ni],
                          rings_v[j + 1][ni], rings_v[j + 1][i]))
    bm.faces.new(tuple(reversed(rings_v[-1])))
    return finalize_bmesh(
        bm, "Crown_BossRise", COLLECTION, ctx.get_material("stone"), ctx,
        kind="landmark", dims=(outer_r * 2, outer_r * 2, height),
        meta={"landmark": "CrownBossSanctum", "element": "smooth_rise",
              "walkable": True, "rise_height_m": height,
              "approach": "SMOOTH_RADIAL", "anchor_unchanged": True},
    )
...
def _boss_button(ctx, center, ground_z):
    ...
    top_z = ground_z + 1.4
    bm = bmesh.new()
    bmesh.ops.create_cylinder(bm, cap_ends=True, vertices=40,
...
def _half_coliseum(ctx, center, ground_z):
    # 28x18 m pocket * 1.15 = 32.2x20.7 m sanctum footprint.
    a = 16.1
    b = 10.35
    wall_t = 0.55
    min_h = 1.0
    ...
    for i in range(segments):
        ...
        # Broken-coliseum height: higher near the crown/back centre.
        ...
        span = max(1.3, min(3.2, a * math.pi / segments * 1.10))
        ...
        created.append(_cube(
            ...
            meta={"landmark": "CrownBossSanctum", "element": "ruined_half_coliseum",
                  "segment": i + 1, "open_direction": "south",
                  "symmetry": "x -> -x"},
        ))
...
def generate(ctx):
    ...
    ground_z = get_height_at_point(Vector((center.x, center.y, 0.0)),
    ...
    print("  -> Crown Sanctum: smooth rise=1.40m | boss button Ø6.40m | ruined half-coliseum=32.2x20.7m | pocket +15%")
    return created
