...
setup_files.py
--------------
Run this once (plain Python or Blender's Python) from the folder where you
want the AetherFlow project to live. It creates the package structure:

    core/       (config.py, context.py, utils.py, layout.py, heightmap.py)
    geometry/   (terrain.py, core_geometry.py, bases.py, roads.py)
    visual/     (materials.py, decorations.py)
    combat/     (combat_cover.py, ambush.py, sightlines.py)
    main.py

and writes each file with the exact contents provided. All modules imported
by main.py are now included — running main.py inside Blender should build
the full map in one go.
"""

import os

# ---------------------------------------------------------------------
# Project root = the directory this script is run from.
# ---------------------------------------------------------------------
ROOT = os.path.dirname(os.path.abspath(__file__))

PACKAGES = ["core", "geometry", "visual", "combat"]
...
# core/context.py
# =====================================================================
FILES["core/context.py"] = '''class MapContext:
...
# core/utils.py
# =====================================================================
FILES["core/utils.py"] = '''import bpy
...
# visual/materials.py
# =====================================================================
FILES["visual/materials.py"] = '''import bpy
...
# =====================================================================
# combat/combat_cover.py
# =====================================================================
FILES["combat/combat_cover.py"] = '''import bmesh
import math
import mathutils
...
# combat/ambush.py
# =====================================================================
FILES["combat/ambush.py"] = '''import bmesh
from mathutils import Vector
...
# =====================================================================
# combat/sightlines.py
# =====================================================================
FILES["combat/sightlines.py"] = '''import bpy
import bmesh
from mathutils import Vector
...
# core/layout.py
# =====================================================================
FILES["core/layout.py"] = '''import math
from mathutils import Vector
...
# core/heightmap.py
# =====================================================================
FILES["core/heightmap.py"] = '''import math
from mathutils import Vector
...
# geometry/terrain.py
# =====================================================================
FILES["geometry/terrain.py"] = '''import bpy
import bmesh
from mathutils import Vector
from core.heightmap import get_height_at_point
...
# =====================================================================
# geometry/core_geometry.py
# =====================================================================
FILES["geometry/core_geometry.py"] = '''import bmesh
from mathutils import Vector
from core.layout import polar
from core.heightmap import get_height_at_point
from core.utils import finalize_bmesh
...
# geometry/bases.py
# =====================================================================
FILES["geometry/bases.py"] = '''import bmesh
from mathutils import Vector
from core.heightmap import get_height_at_point
...
# geometry/roads.py
# =====================================================================
FILES["geometry/roads.py"] = '''import bmesh
from mathutils import Vector
from core.heightmap import get_height_at_point
...
# =====================================================================
# visual/decorations.py
# =====================================================================
FILES["visual/decorations.py"] = '''import bmesh
from mathutils import Vector
from core.heightmap import get_height_at_point
from core.utils import finalize_bmesh
...
# =====================================================================
# main.py  (project root, next to core/ geometry/ visual/ combat/)
# =====================================================================
...
# Adds the project folder to sys.path
dir_path = os.path.dirname(bpy.data.filepath)
if not dir_path or dir_path == "":
    # Set this to your project folder if the .blend file hasn't been saved yet
    dir_path = "C:/Program Files/Blender Foundation/Blender 5.2/AetherFlow"
...
# 2. FORCE RELOAD (so edits are picked up immediately)
importlib.reload(core.config)
importlib.reload(core.context)
importlib.reload(core.utils)
importlib.reload(core.layout)
importlib.reload(core.heightmap)
importlib.reload(visual.materials)
importlib.reload(visual.decorations)
importlib.reload(geometry.terrain)
importlib.reload(geometry.core_geometry)
importlib.reload(geometry.bases)
importlib.reload(geometry.roads)
importlib.reload(combat.combat_cover)
importlib.reload(combat.ambush)
importlib.reload(combat.sightlines)

def run():
    print("--- GENERATING AETHER FLOW BLOCKOUT 0.4.0 ---")

    # 3. Init context
    ctx = core.context.MapContext(core.config.CONFIG)

    # 4. Clear scene / setup collections & materials
    core.utils.clear_scene()
    core.utils.setup_collections(ctx)
    visual.materials.setup_materials(ctx)

    # 5. Build layout / heights
    ctx.layout = core.layout.build_layout(ctx.config)

    # 6. Generate all geometry
    ...
    print("--- SUCCESS: AETHER FLOW MAP GENERATED SUCCESSFULLY ---")
...
def main():
    ...
    print("\nDone. Created:", ", ".join(PACKAGES), "+ main.py")
    print(f"All {len(FILES)} project files written — main.py should run with no missing imports.")
