...
def _python_bootstrap(ue_bridge_dir: Path, manifest_path: Path, clear_previous: bool) -> str:
    ...
    return (
        ...
        f"print(sync_manifest(r'{manifest}', clear_previous={flag}))"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--keep-generated", action="store_true", help="Do not remove previous AetherFlow bridge markers")
    ...
    try:
        info = client.info()
        code = _python_bootstrap(ue_python_dir, manifest, not args.keep_generated)
