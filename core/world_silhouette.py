...
def _rock(name, center, radius, height, material, ctx, flatten=0.82, sides=10):
    ...
    for ring in range(rings):
        ...
        for i in range(sides):
            ...
            verts.append(bm.verts.new((radius * ring_scale * wobble * math.cos(a), radius * flatten * ring_scale * wobble * math.sin(a), z)))
    ...
    x, y, _ = center
    obj = finalize_bmesh(bm, name, COLLECTION, material, ctx, kind="visual_silhouette",
        dims=(radius*2.0, radius*flatten*2.0, height),
        meta={"visual_only": True, "navigation_blocker": False, "los_blocker": False, "symmetry_role": "visual_macro"})
    obj.location = Vector((x, y, _terrain_z(ctx, x, y) + height * 0.50 - height * 0.10))
    return obj
...
def _ridge(name, center, width, depth, height, material, ctx):
    ...
    verts = [bm.verts.new((-x,-y,0)), bm.verts.new((x,-y,0)), bm.verts.new((x,y,0)), bm.verts.new((-x,y,0)),
    ...
    cx, cy, _ = center
    obj = finalize_bmesh(bm, name, COLLECTION, material, ctx, kind="visual_silhouette",
        dims=(width, depth, height), meta={"visual_only": True, "navigation_blocker": False,
        "los_blocker": False, "symmetry_role": "visual_macro"})
    obj.location = Vector((cx, cy, _terrain_z(ctx, cx, cy)))
    return obj


def _pair_rock(ctx, material, name, x, y, radius, height, flatten):
    return [_rock(name+"_L", (x,y,0.0), radius, height, material, ctx, flatten),
            _rock(name+"_R", (-x,y,0.0), radius, height, material, ctx, flatten)]


def _pair_ridge(ctx, material, name, x, y, width, depth, height):
    return [_ridge(name+"_L", (x,y,0.0), width, depth, height, material, ctx),
...
def audit_world_silhouette_symmetry(objects):
    ...
    for obj in objects:
        ...
        if pair is None:
            failures.append((obj.name, "missing counterpart")); continue
...
def generate_world_silhouette(ctx):
    """Build macro visual framing; visual objects never block gameplay/navigation."""
    ...
    created = []
    rock_specs = [
        ("OuterCliff01",88.0,22.0,7.0,11.0,0.72),("OuterCliff02",91.0,-8.0,8.0,13.0,0.78),
        ("OuterCliff03",83.0,-43.0,7.5,10.0,0.76),("NorthCliffWing",58.0,84.0,7.5,12.0,0.80),
        ("CrownFrame",38.0,52.0,6.0,9.0,0.84),("BaseFrame",42.0,-88.0,6.5,8.5,0.82),
        ("AetherCoreFrame",22.0,1.5,5.5,8.0,0.88)]
    for item in rock_specs: created.extend(_pair_rock(ctx, material, *item))
    ridge_specs = [("OuterRidgeNorth",72.0,74.0,28.0,10.0,7.0),("OuterRidgeSouth",76.0,-74.0,24.0,11.0,6.5),("VisualShoulder",86.0,48.0,18.0,16.0,6.0)]
    for item in ridge_specs: created.extend(_pair_ridge(ctx, material, *item))
    report = audit_world_silhouette_symmetry(created)
    print("  -> VIZ-01 world silhouette: objects={} | symmetry={} | max_error={:.6f}m".format(len(created), "PASS" if report["passed"] else "FAIL", report["max_error_m"]))
