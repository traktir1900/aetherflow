...
CONFIG = {
    ...
    "pockets": {
        ...
        # margin kept clear between the interior cover optimiser's usable area
        # and the pocket outline (matches core/cover_analysis "t" in tests)
        "cover_margin": _s(4.2),           # 1.4 m
        # max tolerated mismatch between mirror partners (fairness validation)
        "fairness_tolerance":  _s(1.5),     # 0.5 m
        # ---- gameplay cover optimiser (computed, not a fixed object count) ----
        "cover": {
            "pct_max": 0.15,          # cover may occupy at most 15% of the floor
            "min_passage": _s(9.0),   # 3 m — no passage may get narrower (no chokes)
            "max_objects": 3,         # spec v0.6.2: 2-3 cover rocks per pocket
            "min_score": 1.5,         # drop cover without genuine gameplay value
            "w_los": 3.0,             # COVER_SCORE weights
            "w_flank": 1.0,
            "w_defensive": 1.5,
            "w_movement": 3.0,
            "w_choke": 4.0,
    ...
    # Global outer natural wall / ellipse configuration.
    # Map is 210x210m; keep the previously approved wall mass/height and
    # restore the original full inward wall depth (no 1/3 thinning).
    "outer_boundary": {
        "enabled": True,
        "segments": 48,
