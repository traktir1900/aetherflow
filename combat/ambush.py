...
def generate_south_rift_ambush(ctx):
    ...
    offsets = [Vector((-15.0, 5.0, 0)), Vector((15.0, -5.0, 0))]
    for i, off in enumerate(offsets):
        r_pos = south_mid + off
        r_pos.z = rift_z
        bm = bmesh.new()
        bmesh.ops.create_icosphere(bm, subdivisions=1, radius=8.0)
        bmesh.ops.scale(bm, vec=Vector((1.4, 0.9, 1.1)), verts=bm.verts)
        bmesh.ops.translate(bm, verts=bm.verts, vec=r_pos + Vector((0, 0, 3.75)))
