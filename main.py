...
def _remove_named_prefix_objects(ctx, prefixes):
    """Remove explicit generated prop families, case-insensitively."""
    import bpy
    prefixes = tuple(str(p).lower() for p in prefixes)
    removed = []
    for obj in list(bpy.data.objects):
        if any(obj.name.lower().startswith(prefix) for prefix in prefixes):
            removed.append(obj.name)
            bpy.data.objects.remove(obj, do_unlink=True)
    if ctx is not None and hasattr(ctx, "generated_objects") and removed:
        removed_set = set(removed)
...
def _install_crown_sanctum():
    ...
    def wrapper(*args, **kwargs):
        ...
        if ctx is not None:
            ...
            removed_crown_capture = _remove_named_prefix_objects(
                ctx,
                ("CapturePlatform_Crown",),
            )
            if removed_crown_capture:
                print("  -> removed Crown capture platform: {}".format(removed_crown_capture))
        return result
...
print("[CROWN SANCTUM] PATCH ACTIVE: Lord/BossButton sanctuary; Crown capture platform removed")
...
def run():
    ...
    result = core.pipeline.run_pipeline()
    removed_base_shops = _remove_named_prefix_objects(None, ("Blue_Shop", "Red_Shop"))
    if removed_base_shops:
        print("  -> FINAL BASE SHOP CLEANUP: removed {}".format(removed_base_shops))
    return result
